...
function checksExistsUserAccount(request, response, next) {
  ...
  const user = users.find((h) => h.username === username)
  if (!user) {
      return response.status(404).json({error: 'custuer not found'})
  }
...
app.post('/users', (request, response) => {
  ...
  if (exist) {
    return response.status(400).json({error: "ja existe"})
  }
...
app.delete('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  const todo = username.todos.findIndex(h => h.id === id)
